...
#define QUERY_PLANB_DOWNLOADS_BY_USER "drop table if exists esgf_dashboard.downloads_by_user; create table esgf_dashboard.downloads_by_user as (SELECT COUNT(*) AS numdownloads, SUM(fixed_log.size)/1024/1024/1024 AS downloadeddata, fixed_log.user_id_hash FROM (SELECT file.url, log.user_id_hash, max(file.size) AS size FROM esgf_node_manager.access_logging AS log JOIN public.file_version AS file ON (log.url LIKE '%.nc' AND log.url = file.url) WHERE log.success AND log.duration > 1000 GROUP BY file.url, log.user_id_hash) AS fixed_log GROUP BY user_id_hash);"
