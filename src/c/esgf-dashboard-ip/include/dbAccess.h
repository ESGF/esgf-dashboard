...
//27-01-2016 #define QUERY_PLANB_SUMMARY_DB_TMP "drop table if exists esgf_dashboard.finaldw_planb_tmp; create table esgf_dashboard.finaldw_planb_tmp as (SELECT EXTRACT (YEAR FROM (TIMESTAMP WITH TIME ZONE 'epoch' + fixed_log.date_fetched * INTERVAL '1 second')) AS year, EXTRACT (MONTH FROM (TIMESTAMP WITH TIME ZONE 'epoch' + fixed_log.date_fetched * INTERVAL '1 second')) AS month, COUNT(*) AS downloads, COUNT(distinct url) AS files, COUNT(distinct user_id_hash) AS users, TO_CHAR(SUM(fixed_log.size)/1024/1024/1024, '9,999,999.99') AS gb FROM (SELECT file.url, log.user_id_hash, max(log.date_fetched) AS date_fetched, max(file.size) AS size FROM esgf_node_manager.access_logging AS log JOIN public.file_version AS file ON (log.url LIKE '%cmip5%.nc' AND log.url = file.url) WHERE log.success AND log.duration > 1000 GROUP BY file.url, log.user_id_hash) AS fixed_log GROUP BY year, month ORDER BY year, month);"

//25-01-2016 #define QUERY_PLANB_SUMMARY_DB_TMP "drop table if exists esgf_dashboard.finaldw_planb_tmp; create table esgf_dashboard.finaldw_planb_tmp as (SELECT EXTRACT (YEAR FROM (TIMESTAMP WITH TIME ZONE 'epoch' + fixed_log.date_fetched * INTERVAL '1 second')) as year, EXTRACT (MONTH FROM (TIMESTAMP WITH TIME ZONE 'epoch' + fixed_log.date_fetched * INTERVAL '1 second')) as month, count(*) as downloads, count(distinct url) as files, count(distinct user_id_hash) as users, sum(fixed_log.size)/1024/1024/1024 as gb FROM (SELECT file.url, log.user_id_hash, max(log.date_fetched) as date_fetched, max(file.size) as size FROM esgf_node_manager.access_logging as log join public.file_version as file on (log.url LIKE '%.nc' AND lower(regexp_replace(log.url, E'^.*/(cmip5/.*\.nc)$', E'\\1')) = lower(regexp_replace(file.url, E'^.*/(cmip5/.*\.nc)$', E'\\1'))) where log.success and log.duration > 1000 group by file.url,log.user_id_hash) as fixed_log group by year,month order by year,month);"

//27-01-2016 #define QUERY_PLANB_ADD_HOSTNAME_COLUMN "alter table esgf_dashboard.finaldw_planb_tmp add column host varchar(1024);"
#define QUERY_PLANB_ADD_HOSTNAME_COLUMN "alter table esgf_dashboard.%s_tmp add column host varchar(1024);"
//27-01-2016 #define QUERY_PLANB_ADD_HOSTNAME_VALUE "update esgf_dashboard.finaldw_planb_tmp set host='%s'"
...
//27-01-2016 #define QUERY_PLANB_SUMMARY_DB "drop table if exists esgf_dashboard.finaldw_planb; alter table esgf_dashboard.finaldw_planb_tmp rename to finaldw_planb;"
#define QUERY_PLANB_SUMMARY_DB "drop table if exists esgf_dashboard.%s; alter table esgf_dashboard.%s_tmp rename to %s;"
#define QUERY_PLANB_SUMMARY_DB_CONTINENT "drop table if exists esgf_dashboard.%s_continent; alter table esgf_dashboard.%s_continent_tmp rename to %s_continent;"
//29-01-2016 add "DOWNLOADS BY IDP"
...
#define QUERY_PLANB_DOWNLOADS_BY_IDP_CONTINENT "drop table if exists esgf_dashboard.downloads_by_idp_continent; create table esgf_dashboard.downloads_by_idp_continent as (SELECT COUNT(*) AS numdownloads, log.user_idp, cl.continent FROM esgf_dashboard.dashboard_queue AS log JOIN esgf_dashboard.client_stats_dm AS cl ON (log.remote_addr=cl.ip) GROUP BY log.user_idp, cl.continent);"
//29-01-2016 add "DOWNLOADS BY USER"   
...
#define QUERY_PLANB_DOWNLOADS_BY_USER_CONTINENT "drop table if exists esgf_dashboard.downloads_by_user_continent; create table esgf_dashboard.downloads_by_user_continent as (SELECT COUNT(*) AS numdownloads, SUM(fixed_log.size)/1024/1024/1024 AS downloadeddata, fixed_log.user_id_hash, fixed_log.continent FROM (SELECT file.url, log.user_id_hash, max(file.size) AS size, cl.continent FROM esgf_dashboard.dashboard_queue AS log JOIN esgf_dashboard.client_stats_dm AS cl ON (log.remote_addr=cl.ip) JOIN public.file_version AS file ON (UPPER(log.url_path) LIKE '%.NC' AND log.url_path=file.url) WHERE log.success AND log.duration > 1000 GROUP BY file.url, log.user_id_hash, cl.continent) AS fixed_log GROUP BY user_id_hash, continent);"
