...
#define HAST_TABLE_CORDEX_DOWNLOAD		300
...
#define HAST_TABLE_OBS_DIM_DATASET	                256	
...
#define HAST_TABLE_OBS_DIM_REALM			128
...
// HashTables dimensions - CORDEX Project
#define HAST_TABLE_CORDEX_DIM_MODEL	        	64
#define HAST_TABLE_CORDEX_DIM_EXPERIMENT	        64
#define HAST_TABLE_CORDEX_BRIDGE_MODEL		64	
#define HAST_TABLE_CORDEX_BRIDGE_MODEL_TMP	        64	
#define HAST_TABLE_CORDEX_BRIDGE_EXPERIMENT		64	
#define HAST_TABLE_CORDEX_BRIDGE_EXPERIMENT_TMP	        64	
...
#define QUERY_OPEN_TRANSACTION  "start transaction; lock esgf_dashboard.cross_dim_date ; lock esgf_dashboard.cross_dim_geolocation ; lock esgf_dashboard.cross_dim_project ; lock esgf_dashboard.cross_bridge_project ; lock esgf_dashboard.cross_fact_download ; lock esgf_dashboard.cmip5_dim_model ; lock esgf_dashboard.cmip5_bridge_model; lock esgf_dashboard.cordex_dim_model; lock esgf_dashboard.cordex_bridge_model;"
...
int check_cross_project (PGconn *conn, struct dataset_project ***datasetproj, char* esgf_node, int res_rep)
{
  ...
  int exp_group_key=0;
  int exp_group_key_c=0;
  int model_group_key=0;
  int model_group_key_c=0;
  int geo_id = 0;
  int geo_obs_id = 0;
  char *proj_name = NULL;
  ...
  HASHTBL *hashtbl_cordex_fact_download;
  ...
  HASHTBL *hashtbl_obs_dim_model;
  HASHTBL *hashtbl_obs_bridge_model;
  HASHTBL *hashtbl_obs_bridge_model_tmp;
  HASHTBL *hashtbl_obs_dim_experiment;
  HASHTBL *hashtbl_obs_bridge_experiment;
  HASHTBL *hashtbl_obs_bridge_experiment_tmp;
      ...
      for(size3=0; (*datasetproj)[cnt]->first[size2]->first[size3]!=NULL; size3++)
      {
        if((*datasetproj)[cnt]->first[size2]->first[size3]!=NULL)
        {
          if((*datasetproj)[cnt]->first[size2]->first[size3]->name)
          {
            ...
            if(strcmp((*datasetproj)[cnt]->first[size2]->first[size3]->name, "driving_model")==0)
            {
              size_model_cord=(*datasetproj)[cnt]->first[size2]->first[size3]->size;
              model_cord=(char **)calloc(size_model_cord+1, sizeof(char*));                    
              for(size4=0; (*datasetproj)[cnt]->first[size2]->first[size3]->value[size4]!=NULL; size4++)
              {                    
                model_cord[size4]=strdup((*datasetproj)[cnt]->first[size2]->first[size3]->value[size4]);
              }
              model_cord[size4]=NULL;
            }
...
// create and populate hashtables if not yetif (!create_populate_done)
	...
	hashtbl_destroy (hashtbl_cmip5_fact_download);
      ...
      pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CORDEX DOWNLOAD\n");
      if (!(hashtbl_cordex_fact_download = hashtbl_create (HAST_TABLE_CORDEX_DOWNLOAD, NULL)))
      {
        pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for RSSFEED [skip parsing]\n");
	hashtbl_destroy (hashtbl_cross_dim_date);
	hashtbl_destroy (hashtbl_cross_dim_geolocation);
	hashtbl_destroy (hashtbl_cross_dim_project);
        ...
        hashtbl_destroy (hashtbl_obs4mips_fact_download);
        hashtbl_destroy (hashtbl_cmip5_fact_download);
        hashtbl_destroy (hashtbl_cordex_fact_download);
      }
    }
	...
	pmesg(LOG_DEBUG,__FILE__,__LINE__,"I got the geolocation info from the DB (estimation)\n");
        ...
        resp_res=submit_query_res (conn, select_geo_query, &res_geo);
   ...
   for(size2=0; (*datasetproj)[cnt]->first[size2]!=NULL; size2++)
   {
	 ...
	 if (!(hashtbl_obs_dim_model = hashtbl_create (HAST_TABLE_CMIP5_DIM_MODEL, NULL)))
	 {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
           ...
           hashtbl_destroy (hashtbl_obs_dim_geolocation);
           hashtbl_destroy (hashtbl_obs4mips_fact_download);
           hashtbl_destroy (hashtbl_obs_dim_model);
	   //continue;
	 }
         pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CMIP5 DIM EXPERIMENT\n");
         if (!(hashtbl_obs_dim_experiment = hashtbl_create (HAST_TABLE_CMIP5_DIM_EXPERIMENT, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
           ...
           hashtbl_destroy (hashtbl_obs4mips_fact_download);
           hashtbl_destroy (hashtbl_obs_dim_model);
           hashtbl_destroy (hashtbl_obs_dim_experiment);
           //continue;
         }	       
         pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CMIP5 BRIDGE MODEL\n");
         if (!(hashtbl_obs_bridge_model = hashtbl_create (HAST_TABLE_CMIP5_BRIDGE_MODEL, NULL)))
	 {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
           ...
           hashtbl_destroy (hashtbl_obs_dim_model);
           hashtbl_destroy (hashtbl_obs_dim_experiment);
           hashtbl_destroy (hashtbl_obs_bridge_model);
         }
         pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the TMP hashtable for CMIP5 BRIDGE MODEL\n");
         if (!(hashtbl_obs_bridge_model_tmp = hashtbl_create (HAST_TABLE_CMIP5_BRIDGE_MODEL_TMP, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
	   ...
	   hashtbl_destroy (hashtbl_cross_fact_download);
           ...
           hashtbl_destroy (hashtbl_obs_dim_experiment);
           hashtbl_destroy (hashtbl_obs_bridge_model);
           hashtbl_destroy (hashtbl_obs_bridge_model_tmp);
         }
	 pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CMIP5 BRIDGE EXPERIMENT\n");
	 if (!(hashtbl_obs_bridge_experiment = hashtbl_create (HAST_TABLE_CMIP5_BRIDGE_EXPERIMENT, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
           ...
           hashtbl_destroy (hashtbl_obs_bridge_model);
           hashtbl_destroy (hashtbl_obs_bridge_model_tmp);
           hashtbl_destroy (hashtbl_obs_bridge_experiment);
         }
	 pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the TMP hashtable for CMIP5 BRIDGE EXPERIMENT\n");
	 if (!(hashtbl_obs_bridge_experiment_tmp = hashtbl_create (HAST_TABLE_CMIP5_BRIDGE_EXPERIMENT_TMP, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
	   ...
	   hashtbl_destroy (hashtbl_obs_bridge_time_frequency);
           ...
           hashtbl_destroy (hashtbl_obs_bridge_model_tmp);
           hashtbl_destroy (hashtbl_obs_bridge_experiment);
           hashtbl_destroy (hashtbl_obs_bridge_experiment_tmp);
         }
	 if (!(hashtbl_cmip5_fact_download = hashtbl_create (HAST_TABLE_CMIP5_DOWNLOAD, NULL)))
	 {
	   pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
       ...
       if(strcmp(proj_name, "CORDEX")==0)
       {
         create_populate_done=4;
	 pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CORDEX DIM MODEL\n");
	 if (!(hashtbl_obs_dim_model = hashtbl_create (HAST_TABLE_CORDEX_DIM_MODEL, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
	   ...
	   //continue;
         ...
         pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CORDEX DIM EXPERIMENT\n");
         if (!(hashtbl_obs_dim_experiment = hashtbl_create (HAST_TABLE_CORDEX_DIM_EXPERIMENT, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
	   ...
	   //continue;
         ...
         pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CORDEX BRIDGE MODEL\n");
         if (!(hashtbl_obs_bridge_model = hashtbl_create (HAST_TABLE_CORDEX_BRIDGE_MODEL, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
	   hashtbl_destroy (hashtbl_cross_dim_geolocation);
	   ...
	   hashtbl_destroy (hashtbl_obs_dim_file);
         ...
         pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the TMP hashtable for CORDEX BRIDGE MODEL\n");
         if (!(hashtbl_obs_bridge_model_tmp = hashtbl_create (HAST_TABLE_CORDEX_BRIDGE_MODEL_TMP, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
	 ...
	 pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the hashtable for CORDEX BRIDGE EXPERIMENT\n");
	 if (!(hashtbl_obs_bridge_experiment = hashtbl_create (HAST_TABLE_CORDEX_BRIDGE_EXPERIMENT, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
	 ...
	 pmesg(LOG_DEBUG,__FILE__,__LINE__,"Creating the TMP hashtable for CORDEX BRIDGE EXPERIMENT\n");
	 if (!(hashtbl_obs_bridge_experiment_tmp = hashtbl_create (HAST_TABLE_CORDEX_BRIDGE_EXPERIMENT_TMP, NULL)))
         {
           pmesg(LOG_WARNING,__FILE__,__LINE__,"ERROR: hashtbl_create() failed for USES [skip parsing]\n");
           hashtbl_destroy (hashtbl_cross_dim_date);
           hashtbl_destroy (hashtbl_cross_dim_geolocation);
         ...
         if (!(hashtbl_cordex_fact_download = hashtbl_create (HAST_TABLE_CORDEX_DOWNLOAD, NULL)))
         {
           ...
           hashtbl_destroy (hashtbl_cordex_fact_download);
	          ...
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_DIM_MODEL,
				   &hashtbl_obs_dim_model);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_BRIDGE_MODEL,
				   &hashtbl_obs_bridge_model);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_BRIDGE_MODEL_TMP,
				   &hashtbl_obs_bridge_model_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_DIM_EXPERIMENT,
				   &hashtbl_obs_dim_experiment);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_BRIDGE_EXPERIMENT,
				   &hashtbl_obs_bridge_experiment);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_BRIDGE_EXPERIMENT_TMP,
				   &hashtbl_obs_bridge_experiment_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CMIP5_DOWNLOAD,
				   &hashtbl_cmip5_fact_download);
	        }
              if(strcmp(proj_name, "CORDEX")==0)
              {
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_DATASET,
				   &hashtbl_obs_dim_dataset);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_INSTITUTE,
				   &hashtbl_obs_dim_institute);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_VARIABLE,
				   &hashtbl_obs_dim_variable);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_TIME_FREQUENCY,
				   &hashtbl_obs_dim_time_frequency);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_INSTITUTE,
				   &hashtbl_obs_bridge_institute);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_INSTITUTE_TMP,
				   &hashtbl_obs_bridge_institute_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_VARIABLE,
				   &hashtbl_obs_bridge_variable);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_VARIABLE_TMP,
				   &hashtbl_obs_bridge_variable_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_TIME_FREQUENCY,
				   &hashtbl_obs_bridge_time_frequency);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_TIME_FREQUENCY_TMP,
				   &hashtbl_obs_bridge_time_frequency_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_DATE,
				   &hashtbl_obs_dim_date);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_GEOLOCATION,
				   &hashtbl_obs_dim_geolocation);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_MODEL,
				   &hashtbl_obs_dim_model);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_MODEL,
				   &hashtbl_obs_bridge_model);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_MODEL_TMP,
				   &hashtbl_obs_bridge_model_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DIM_EXPERIMENT,
				   &hashtbl_obs_dim_experiment);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_EXPERIMENT,
				   &hashtbl_obs_bridge_experiment);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_BRIDGE_EXPERIMENT_TMP,
				   &hashtbl_obs_bridge_experiment_tmp);
	          populate_hash_table (conn, QUERY_GET_LIST_OF_CORDEX_DOWNLOAD,
				   &hashtbl_cordex_fact_download);
	        }
          }
	  else
	  ...
	  {		// add host entry in DB (and hashtable too) without geolocation information
             ...
             if(strcmp(proj_name, "CORDEX")==0)
                snprintf (insert_new_date_query, sizeof (insert_new_date_query), QUERY_INSERT_NEW_CORDEX_DATE, buf, buf1, buf2);
	     ...
	     submit_query (conn, insert_new_date_query);
             ...
             if(strcmp(proj_name, "CORDEX")==0)
	         snprintf(select_id_date_query, sizeof (select_id_date_query), QUERY_GET_CORDEX_DATE_ID, buf);
		 ...
		 pmesg(LOG_DEBUG,__FILE__,__LINE__,"[OUTPUT_LATITUDE=%f]\n",geo_output.latitude);
		     ...
		     resp_res=submit_query_res (conn, select_geo_query, &res_geo);
		       ...
		       snprintf (insert_geo_query, sizeof (insert_geo_query),QUERY_INSERT_CMIP5_DIM_GEOLOCATION, lat, lon, atoi(country_id));
                     ...
                     if((country_id) && (strcmp(proj_name, "CORDEX")==0))
		       snprintf (insert_geo_query, sizeof (insert_geo_query),QUERY_INSERT_CORDEX_DIM_GEOLOCATION, lat, lon, atoi(country_id));
	                ...
	                snprintf(select_id_geo_query, sizeof (select_id_geo_query), QUERY_GET_GEO_CMIP5_ID, lat, lon);
                     if(strcmp(proj_name, "CORDEX")==0)
	                snprintf(select_id_geo_query, sizeof (select_id_geo_query), QUERY_GET_GEO_CORDEX_ID, lat, lon);
		...
		//pmesg(LOG_ERROR,__FILE__,__LINE__,"Esgf-lookup error\n");
         ...
         {  
            ...
            else
            {
               ...
               if(strcmp(proj_name, "CORDEX")==0)
	           snprintf (insert_inst_query, sizeof (insert_inst_query),QUERY_INSERT_CORDEX_DIM_INSTITUTE,institute[i]);
               ...
               if(strcmp(proj_name, "CORDEX")==0)
	           snprintf(select_id_inst_query, sizeof (select_id_inst_query), QUERY_GET_INSTITUTE_CORDEX_ID,institute[i]);
         ...
         if(miss_inst==1)
         {
            ...
            if(strcmp(proj_name, "CORDEX")==0)
                 resp=submit_query_res (conn, QUERY_INST_GET_MAX_GROUP_CORDEX_ID,&res2);
            if(resp!=-1)
            {
               for (i = 0; i < PQntuples(res2); i++)
               {
            ...
            for(i=0; i<size_institute;i++)
            {
                ...
                if(strcmp(proj_name, "CORDEX")==0)
                    snprintf (insert_group_inst_query, sizeof (insert_group_inst_query),QUERY_INSERT_CORDEX_BRIDGE_INSTITUTE,array_id_inst[i],max_group_id+1);
        ...
        free(array_id_inst);
         ...
         {  
            ...
            else
            {
	          ...
	          snprintf (insert_var_query, sizeof (insert_var_query),QUERY_INSERT_CMIP5_DIM_VARIABLE, cf_standard_name[i], variable_long_name[i], variable[i]);
               if(strcmp(proj_name, "CORDEX")==0)
	          snprintf (insert_var_query, sizeof (insert_var_query),QUERY_INSERT_CORDEX_DIM_VARIABLE, cf_standard_name[i], variable_long_name[i], variable[i]);
	          ...
	          snprintf(select_id_var_query, sizeof (select_id_var_query), QUERY_GET_VARIABLE_CMIP5_ID,cf_standard_name[i], variable_long_name[i], variable[i]);
               if(strcmp(proj_name, "CORDEX")==0)
	          snprintf(select_id_var_query, sizeof (select_id_var_query), QUERY_GET_VARIABLE_CORDEX_ID,cf_standard_name[i], variable_long_name[i], variable[i]);
         ...
         if(miss_var==1)
         {
            ...
            if(strcmp(proj_name, "CORDEX")==0)
                resp=submit_query_res (conn, QUERY_VAR_GET_MAX_GROUP_CORDEX_ID,&res2);
            if(resp!=-1)
            {
               for (i = 0; i < PQntuples(res2); i++)
               {
            ...
            for(i=0; i<size_variable;i++)
            {
                ...
                if(strcmp(proj_name, "CORDEX")==0)
                   snprintf (insert_group_var_query, sizeof (insert_group_var_query),QUERY_INSERT_CORDEX_BRIDGE_VARIABLE,array_id_var[i],max_group_id+1);
         ...
         {  
            ...
            else
            {
	         ...
	         snprintf (insert_time_freq_query, sizeof (insert_time_freq_query),QUERY_INSERT_CMIP5_DIM_TIME_FREQUENCY, time_frequency[i]);
               if(strcmp(proj_name, "CORDEX")==0)
	         snprintf (insert_time_freq_query, sizeof (insert_time_freq_query),QUERY_INSERT_CORDEX_DIM_TIME_FREQUENCY, time_frequency[i]);
	         ...
	         snprintf(select_id_time_freq_query, sizeof (select_id_time_freq_query), QUERY_GET_TIME_FREQ_CMIP5_ID,time_frequency[i]);
               if(strcmp(proj_name, "CORDEX")==0)
	         snprintf(select_id_time_freq_query, sizeof (select_id_time_freq_query), QUERY_GET_TIME_FREQ_CORDEX_ID,time_frequency[i]);
         ...
         if(miss_time_freq==1)
         {
            ...
            if(strcmp(proj_name, "CORDEX")==0)
               resp=submit_query_res (conn, QUERY_TIME_FREQ_GET_MAX_GROUP_CORDEX_ID,&res2);
            if(resp!=-1)
            {
               for (i = 0; i < PQntuples(res2); i++)
               {
            ...
            for(i=0; i<size_time;i++)
            {
                ...
                if(strcmp(proj_name, "CORDEX")==0)
                   snprintf (insert_group_time_freq_query, sizeof (insert_group_time_freq_query),QUERY_INSERT_CORDEX_BRIDGE_TIME_FREQUENCY,array_id_time_freq[i],max_group_id+1);
      ...
      if((strcmp(proj_name, "CMIP5")==0) || (strcmp(proj_name, "OBS4MIPS")==0))
      {
      ...
      if(strcmp(proj_name, "CMIP5")==0)
      {
         ...
         {  
            ...
            if (hashtbl_result = hashtbl_get (hashtbl_obs_dim_model, model[i]))
            {
	       ...
	       sprintf (model_id_str, "%ld", model_id);
               hashtbl_insert (hashtbl_obs_dim_model,model[i],model_id_str);
         ...
         if(miss_model==1)
         {
            ...
            for(i=0; i<size_model;i++)
            {
                ...
                sprintf(max_group_id_str, "%d", max_group_id+1);
                hashtbl_insert (hashtbl_obs_bridge_model,array_model_id_str,max_group_id_str);
                pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",array_model_id_str,max_group_id_str);
            } 
            model_group_key= max_group_id+1;
            hashtbl_insert (hashtbl_obs_bridge_model_tmp,max_group_id_str,model_key);
            pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",max_group_id_str,model_key);
         }
         else
         {
            if (hashtbl_result = hashtbl_get (hashtbl_obs_bridge_model_tmp, model_key))
            {
      ...
      if(strcmp(proj_name, "CMIP5")==0)
      {
         ...
         {  
            int exp_id=0;
            if (hashtbl_result = hashtbl_get (hashtbl_obs_dim_experiment, experiment[i]))
            {
	       ...
	       sprintf (exp_id_str, "%ld", exp_id);
               hashtbl_insert (hashtbl_obs_dim_experiment,experiment[i],exp_id_str);
         ...
         if(miss_exp==1)
         {
            ...
            for(i=0; i<size_exp;i++)
            {
                ...
                sprintf(max_group_id_str, "%d", max_group_id+1);
                hashtbl_insert (hashtbl_obs_bridge_experiment,array_exp_id_str,max_group_id_str);
                pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",array_exp_id_str,max_group_id_str);
            } 
            exp_group_key= max_group_id+1;
            hashtbl_insert (hashtbl_obs_bridge_experiment_tmp,max_group_id_str,exp_key);
            pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",max_group_id_str,exp_key);
         }
         else
         {
            if (hashtbl_result = hashtbl_get (hashtbl_obs_bridge_experiment_tmp, exp_key))
            {
            ...
            else
            {
                ...
                for(i=0; i<size_exp;i++)
                {
                  ...
                  sprintf(max_group_id_str, "%d", max_group_id+1);
                  hashtbl_insert (hashtbl_obs_bridge_experiment,array_id_exp_str,max_group_id_str);
        ...
        /* cordex_dim_model and bridge */
      if(strcmp(proj_name, "CORDEX")==0)
      {
         int miss_model_c=0;
         int *array_model_id_cord=calloc(size_model_cord, sizeof(int));

         for(i=0; i<size_model_cord; i++)
         {
            int model_id_c=0;
            if (hashtbl_result = hashtbl_get (hashtbl_obs_dim_model, model_cord[i]))
            {
               pmesg(LOG_DEBUG,__FILE__,__LINE__,"Lookup HostTable hit! [%s] [%s]\n", model_cord[i], hashtbl_result);
               model_id_c = atoi (hashtbl_result);
               success_lookup[0]++;
               array_model_id_cord[i]=model_id_c;
            }
            else
            {
               miss_model_c=1;
               char model_id_str_c[128] = { '\0' };
               char select_model_id_c[2048] = { '\0' };
               char insert_model_query_c[2048] = { '\0' };
               char select_model_id_query_c[2048] = { '\0' };
               success_lookup[1]++;
               snprintf (insert_model_query_c, sizeof (insert_model_query_c),QUERY_INSERT_CORDEX_DIM_MODEL, model_cord[i]);
               submit_query (conn, insert_model_query_c);
               snprintf(select_model_id_query_c, sizeof (select_model_id_query_c), QUERY_GET_MODEL_CORDEX_ID,model_cord[i]);
               model_id_c=get_foreign_key_value(conn, select_model_id_query_c);
               array_model_id_cord[i]=model_id_c;
               // add entry to hash table
               sprintf (model_id_str_c, "%ld", model_id_c);
               hashtbl_insert (hashtbl_obs_dim_model,model_cord[i],model_id_str_c);
               pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",model_cord[i], model_id_str_c);
             }
           }
         quickSort(array_model_id_cord,0,size_model_cord-1);
         char model_key_c[2048] = { '\0' };
         ap = model_key_c;
         for(i=0; i<size_model_cord;i++)
         {
                if(i==0 || i==size_model_cord-1)
                   ap += sprintf(ap, "%d", array_model_id_cord[i]);
                else
                   ap += sprintf(ap, ",%d,", array_model_id_cord[i]);
         }
         if(miss_model_c==1)
         {
            PGresult   *res2;
            int resp=0;
            int max_group_id_c=0;
            char insert_group_model_query_c[2048] = { '\0' };
            char array_model_id_str_c[2048] = { '\0' };
            char max_group_id_str_c[2048] = { '\0' };
            resp=submit_query_res (conn, QUERY_MODEL_GET_MAX_GROUP_CORDEX_ID,&res2);
            if(resp!=-1)
            {
               for (i = 0; i < PQntuples(res2); i++)
               {
                  max_group_id_c=atoi(PQgetvalue(res2, i, 0));
               }
               PQclear (res2);
            }
            for(i=0; i<size_model_cord;i++)
            {
                //sicuramente devo aggiungere un nuovo gruppo
                snprintf (insert_group_model_query_c, sizeof (insert_group_model_query_c),QUERY_INSERT_CORDEX_BRIDGE_MODEL,array_model_id_cord[i],max_group_id_c+1);
                submit_query (conn, insert_group_model_query_c);

                sprintf(array_model_id_str_c, "%d", array_model_id_cord[i]);
                sprintf(max_group_id_str_c, "%d", max_group_id_c+1);
                hashtbl_insert (hashtbl_obs_bridge_model,array_model_id_str_c,max_group_id_str_c);
                pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",array_model_id_str_c,max_group_id_str_c);
            }
            model_group_key_c= max_group_id_c+1;
            hashtbl_insert (hashtbl_obs_bridge_model_tmp,max_group_id_str_c,model_key_c);
                        
            pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",max_group_id_str_c,model_key_c);
         }
         else
         {
            if (hashtbl_result = hashtbl_get (hashtbl_obs_bridge_model_tmp, model_key_c))
            {
               pmesg(LOG_DEBUG,__FILE__,__LINE__,"Lookup HostTable hit! [%d] [%s]\n",model_key_c, hashtbl_result);
               model_group_key_c = atol(hashtbl_result);
                success_lookup[0]++;
            }
            else
            {
        ...
        free(array_model_id_cord);
      }

      /* cordex_dim_experiment and bridge */
      ...
      if(strcmp(proj_name, "CORDEX")==0)
      {
         ...
         {  
            ...
            else
            {
               ...
               char select_exp_id_query[2048] = { '\0' };
               success_lookup[1]++;
	       snprintf (insert_exp_query, sizeof (insert_exp_query),QUERY_INSERT_CORDEX_DIM_EXPERIMENT, experiment[i]);
               submit_query (conn, insert_exp_query);
	       snprintf(select_exp_id_query, sizeof (select_exp_id_query), QUERY_GET_EXPERIMENT_CORDEX_ID,experiment[i]);
               exp_id=get_foreign_key_value(conn, select_exp_id_query);
               array_exp_id[i]=exp_id;
	       // add entry to hash table
	       sprintf (exp_id_str, "%ld", exp_id);
         ...
         if(miss_exp==1)
         {
            ...
            char array_exp_id_str[2048] = { '\0' };
            char max_group_id_str[2048] = { '\0' };
            resp=submit_query_res (conn, QUERY_EXPERIMENT_GET_MAX_GROUP_CORDEX_ID,&res2);
            if(resp!=-1)
            {
               for (i = 0; i < PQntuples(res2); i++)
               {
            ...
            {
                //sicuramente devo aggiungere un nuovo gruppo
                snprintf (insert_group_exp_query, sizeof (insert_group_exp_query),QUERY_INSERT_CORDEX_BRIDGE_EXPERIMENT,array_exp_id[i],max_group_id+1);
                submit_query (conn, insert_group_exp_query);
            ...
            exp_group_key_c= max_group_id+1;
            hashtbl_insert (hashtbl_obs_bridge_experiment_tmp,max_group_id_str,exp_key);
            pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",max_group_id_str,exp_key);
         }
         else
         {
            ...
            {
               pmesg(LOG_DEBUG,__FILE__,__LINE__,"Lookup HostTable hit! [%d] [%s]\n",exp_key, hashtbl_result);
               exp_group_key_c = atol(hashtbl_result);
               //printf("experiment_group_key %d\n", exp_group_key_c);
               success_lookup[0]++;
            }
            else
         ...
         if((strcmp(proj_name, "OBS4MIPS")==0) || (strcmp(proj_name, "CMIP5")==0) || (strcmp(proj_name, "CORDEX")==0))
         {
          ...
          {             // add host entry in DB (and hashtable too) without geolocation information
             ...
             if(strcmp(proj_name, "CORDEX")==0)
                 snprintf (insert_new_dataset_query, sizeof (insert_new_dataset_query), QUERY_INSERT_NEW_DATASET_CORDEX, dataset_name, atoi((*datasetproj)[cnt]->version), datetime_start, datetime_stop);
             ...
             if(strcmp(proj_name, "CORDEX")==0)
                snprintf(select_id_dataset_query, sizeof (select_id_dataset_query), QUERY_GET_DATASET_CORDEX_ID, dataset_name);
	     ...
	     {		// add host entry in DB (and hashtable too) without geolocation information
          ...
          if(strcmp(proj_name, "CORDEX")==0)
          {
             ...
             if(strcmp(success_row,"t")==0)
                 ...
                 str_succ=strdup("false");
             char fact_cordex_row[2048] = { '\0' };
             long int fact_cordex_id=0;
             sprintf(fact_cordex_row,"%ld:%s:%d:%s:%s:%s:%s:%s:%s:%d:%d:%d:%d:%d:%d:%d:%d:%d", size_row, str_succ, duration_row, replica, esgf_node, buf3, buf4, user_id_hash_row, user_idp_row, date_obs_id, geo_obs_id, dataset_id_key, time_freq_group_key, var_group_key, exp_group_key_c, model_group_key_c, institute_group_key,(*datasetproj)[cnt]->id_query); 
             free(str_succ);
             if (hashtbl_result = hashtbl_get (hashtbl_cordex_fact_download, fact_cordex_row))
             {
               pmesg(LOG_DEBUG,__FILE__,__LINE__,"Lookup HostTable hit! [%s] [%s]\n",fact_row, hashtbl_result);
               fact_cordex_id = atol (hashtbl_result);
	        success_lookup[0]++;
             }
             else
	     {   // add host entry in DB (and hashtable too) without geolocation information
                 char insert_cordex_fact_download[2048] = { '\0' };
                 char select_id_fact_query[2048] = { '\0' };
                 char fact_cordex_id_str[256] = { '\0' };
                 char fact1_row[2048] = { '\0' };
                 sprintf(fact1_row, "%ld:%s:%d:%s:%s:%s:%s:%s:%s:%d:%d:%d:%d:%d:%d:%d:%d", size_row, success_row, duration_row, replica, esgf_node, buf3, buf4, user_id_hash_row, user_idp_row, date_obs_id, geo_obs_id, dataset_id_key, time_freq_group_key, var_group_key, exp_group_key_c, model_group_key_c, institute_group_key,(*datasetproj)[cnt]->id_query);
                  //printf("fact1_row %s\n", fact1_row);
                  snprintf (insert_cordex_fact_download, sizeof (insert_cordex_fact_download), QUERY_INSERT_CORDEX_FACT_DOWNLOAD,size_row, success_row, duration_row, replica, esgf_node, buf3, buf4, user_id_hash_row, user_idp_row, date_obs_id, geo_obs_id, dataset_id_key, time_freq_group_key, var_group_key, exp_group_key_c, model_group_key_c, institute_group_key,(*datasetproj)[cnt]->id_query);
                 //printf("insert_cmip5_fact_download vale %s\n", insert_cordex_fact_download);
	         submit_query (conn, insert_cordex_fact_download);
                 snprintf(select_id_fact_query, sizeof (select_id_fact_query), QUERY_GET_CORDEX_ROW, size_row, success_row, duration_row, replica, esgf_node, buf3, buf4, user_id_hash_row, user_idp_row, date_obs_id, geo_obs_id, dataset_id_key, time_freq_group_key, var_group_key, exp_group_key_c, model_group_key_c, institute_group_key,(*datasetproj)[cnt]->id_query);

                //printf("select_id_fact_query vale %s\n", select_id_fact_query);
                fact_cordex_id=get_foreign_key_value(conn, select_id_fact_query);
                sprintf (fact_cordex_id_str, "%ld", fact_cordex_id);
                hashtbl_insert (hashtbl_cordex_fact_download,fact_cordex_row,fact_cordex_id_str);
                //printf("hashtable [%s]*****[%s]\n",fact_cmip_row,fact_cmip_id_str);
                pmesg(LOG_DEBUG,__FILE__,__LINE__,"[LookupFailed] Adding new entry in the hashtable [%s] [%s]\n",fact_cordex_row,fact_cordex_id_str);
              }
  ...
  pmesg(LOG_DEBUG,__FILE__,__LINE__,"*********** End parsing routine  ************\n");
     ...
     if(strcmp(proj_name, "CORDEX")==0)
        myfree_array(model_cord,size_model_cord+1);

     if((strcmp(proj_name, "CORDEX")==0)||(strcmp(proj_name, "CMIP5")==0))
    ...
    tm->tm_mday, tm->tm_hour, tm->tm_min, tm->tm_sec);
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
        ...
        else
        {
           ...
           if(PQntuples(res1)!=0)
           {
            ...
            if(resp_res!=-1)
              PQclear (res1);
            resp_res=submit_query_res (conn, select_query,&res2);
     ...
     { 
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_POS, datamart);
           int resp_res=submit_query_res (conn, select_id_dmart_query, &res1);
           if(PQntuples(res1)!=0)
           {
            for (i=0; i < PQntuples(res1); i++)
            {
...
//CORDEX LUIS
     if(strcmp(datamart, "esgf_dashboard.cordex_dmart_clients_host_geolocation")==0)
     {  
        if(dmart_key==0)
        {  
           snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_NEW_CORDEX_DMART_CLIENTS_HOST_TIME_GEOLOCATION,ESGF_HOSTNAME);
           submit_query (conn,insert_dmart_project_host);
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
        ...
        else
        {
           ...
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_POS_DATE);
           int resp_res=submit_query_res (conn, select_query,&res1);
	     ...
	     lon=atof(PQgetvalue(res1, i, 1));
             ...
             snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXIST_POS_DATE, str1_geo, str_geo);
             res_query=submit_query_res (conn, select_query,&res2);

             if(PQntuples(res2)!=0)    
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_UPDATE_INSERT_NEW_CORDEX_DMART_CLIENTS_HOST_TIME_GEOLOCATION, str1_geo, str_geo, str1_geo, str_geo);
             else
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_NEW_CORDEX_DMART_CLIENTS_HOST_TIME_GEOLOCATION, str1_geo, str_geo);
     ...
     if(strcmp(datamart, "esgf_dashboard.cordex_dmart_dataset_host_time")==0)
     {  
        if(dmart_key==0)
        {  
           snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_NEW_CORDEX_DMART_DATASET_HOST_TIME,ESGF_HOSTNAME);
           submit_query (conn,insert_dmart_project_host);
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
        ...
        {
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_DATASET_HOST_TIME, dmart_key);
           int res_query=submit_query_res (conn, select_query,&res1);
           char *dataset_name=NULL;
           int dataset_version=0;
           char *datetime_start=NULL;
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_DAT_DATE, month, year);
           res_query=submit_query_res (conn, select_query,&res1);
           ...
           for (i = 0; i < PQntuples(res1); i++)
           {
             ...
             snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXIST_DAT_DATE, month, year, dataset_name, dataset_version, datetime_start, datetime_stop);
             int res_resp=submit_query_res (conn, select_query,&res2);

             if(PQntuples(res2)!=0)    
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_UPDATE_INSERT_NEW_CORDEX_DMART_DATASET_HOST_TIME, month, year, dataset_name, dataset_version, datetime_start, datetime_stop, month, year, dataset_name, dataset_version, datetime_start, datetime_stop);
             else
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_NEW_CORDEX_DMART_DATASET_HOST_TIME, month, year, dataset_name, dataset_version, datetime_start, datetime_stop);
	     ...
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (update_dmart_project_host, sizeof (update_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_CORDEX_DATASET_HOST_TIME, month,year);
           submit_query (conn,update_dmart_project_host);
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
             dmart_id=get_foreign_key_value(conn, select_id_dmart_query);
             snprintf (update_registry, sizeof (update_registry), QUERY_UPDATE_REGISTRY,realtime, dmart_id,datamart);
     ...
     if(strcmp(datamart, "esgf_dashboard.cordex_dmart_variable_host_time")==0)
     {  
        if(dmart_key==0)
        {  
           snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_NEW_CORDEX_DMART_VARIABLE_HOST_TIME,ESGF_HOSTNAME);
           submit_query(conn,insert_dmart_project_host);
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
        ...
        {
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_VARIABLE_HOST_TIME, dmart_key);
           int res_query=submit_query_res (conn, select_query,&res1);
           char *var_code=NULL;
           char *var_name=NULL;
           char *cf_st=NULL;
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_VAR_DATE, month, year);
           res_query=submit_query_res (conn, select_query,&res1);
           ...
           for (i = 0; i < PQntuples(res1); i++)
           {
             ...
             snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXIST_VAR_DATE, month, year, var_code, var_name,cf_st);
             int res_resp=submit_query_res (conn, select_query,&res2);

             if(PQntuples(res2)!=0)    
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_UPDATE_INSERT_NEW_CORDEX_DMART_VARIABLE_HOST_TIME, month, year, var_code, var_name,cf_st, month, year, var_code, var_name,cf_st);
             else
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_NEW_CORDEX_DMART_VARIABLE_HOST_TIME, month, year, var_code, var_name,cf_st);
	     ...
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (update_dmart_project_host, sizeof (update_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_CORDEX_VARIABLE_HOST_TIME, month,year);
             submit_query (conn,update_dmart_project_host);
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
             dmart_id=get_foreign_key_value(conn, select_id_dmart_query);
             snprintf (update_registry, sizeof (update_registry), QUERY_UPDATE_REGISTRY,realtime, dmart_id,datamart);
     ...
     if(strcmp(datamart, "esgf_dashboard.cordex_dmart_model_host_time")==0)
     {  
        if(dmart_key==0)
        {  
           snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_NEW_CORDEX_DMART_MODEL_HOST_TIME,ESGF_HOSTNAME);
           submit_query (conn,insert_dmart_project_host);
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
        ...
        {
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_MODEL_HOST_TIME, dmart_key);
           int res_query=submit_query_res (conn, select_query,&res1);
           char *model_name=NULL;
           if(PQntuples(res1)!=0)
           {
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_MDL_DATE, month, year);
           res_query=submit_query_res (conn, select_query,&res1);
           ...
           for (i = 0; i < PQntuples(res1); i++)
           {
             ...
             snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXIST_MDL_DATE, month, year, model_name);
             int res_resp=submit_query_res (conn, select_query,&res2);

             if(PQntuples(res2)!=0)    
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_UPDATE_INSERT_NEW_CORDEX_DMART_MODEL_HOST_TIME, month, year, model_name, month, year, model_name);
             else
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_NEW_CORDEX_DMART_MODEL_HOST_TIME, month, year, model_name);
	     ...
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
           ...
           if(res_query!=-1)
             PQclear (res1);
             snprintf (update_dmart_project_host, sizeof (update_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_CORDEX_MODEL_HOST_TIME, month,year);
             submit_query (conn,update_dmart_project_host);
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
             dmart_id=get_foreign_key_value(conn, select_id_dmart_query);
             snprintf (update_registry, sizeof (update_registry), QUERY_UPDATE_REGISTRY,realtime, dmart_id,datamart);
     ...
     if(strcmp(datamart, "esgf_dashboard.cordex_dmart_experiment_host_time")==0)
     {  
        if(dmart_key==0)
        {  
           snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_NEW_CORDEX_DMART_EXPERIMENT_HOST_TIME,ESGF_HOSTNAME);
           submit_query (conn,insert_dmart_project_host);
	   ...
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
        ...
        {
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXPERIMENT_HOST_TIME, dmart_key);
           int res_query=submit_query_res (conn, select_query,&res1);
           char *experiment_name=NULL;
           if(PQntuples(res1)!=0)
           {
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXP_DATE, month, year);
           res_query=submit_query_res (conn, select_query,&res1);
           ...
           for (i = 0; i < PQntuples(res1); i++)
           {
             ...
             snprintf (select_query, sizeof (select_query), QUERY_SELECT_CORDEX_DMART_EXIST_EXP_DATE, month, year, experiment_name);
             int res_resp=submit_query_res (conn, select_query,&res2);

             if(PQntuples(res2)!=0)    
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_UPDATE_INSERT_NEW_CORDEX_DMART_EXPERIMENT_HOST_TIME, month, year, experiment_name, month, year, experiment_name);
             else
                 snprintf (insert_dmart_project_host, sizeof (insert_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_NEW_CORDEX_DMART_EXPERIMENT_HOST_TIME, month, year, experiment_name);
	     ...
	     snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
           ...
           if(res_query!=-1)
             PQclear (res1);
           snprintf (update_dmart_project_host, sizeof (update_dmart_project_host), QUERY_SELECT_INSERT_UPDATE_CORDEX_EXPERIMENT_HOST_TIME, month,year);
           submit_query (conn,update_dmart_project_host);
	   snprintf(select_id_dmart_query, sizeof (select_id_dmart_query), QUERY_GET_DMART_ID, datamart);
           dmart_id=get_foreign_key_value(conn, select_id_dmart_query);
           snprintf (update_registry, sizeof (update_registry), QUERY_UPDATE_REGISTRY,realtime, dmart_id,datamart);
